...
class AnswerExtractor:
    ...
    def filtered(self, question, passages):
        filtered_answers = []
        for passage in passages:
            try:
                filtered_answers = self.nlp(question=question, context=passage)
                filtered_answers['text'] = passage
                filtered_answers.append(filtered_answers)
            except KeyError:
                pass
        filtered_answers.sort(key=operator.itemgetter('score'), reverse=True)
